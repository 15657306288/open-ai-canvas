...
const projectStoryboardShotSchema = z.object({
...
export const toolInputSchemas = {
    ...
    canvas_create_storyboard_shots: z.object({ shots: z.array(projectStoryboardShotSchema).min(1).max(100), x: z.number().optional(), direction: z.enum(["row", "column"]).optional() }),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    canvas_create_storyboard_shots: "将已持久化的分镜镜头投影到画布。必须先调用 project_create_or_update_shots 持久化镜头并获取 shotId，再用返回的 shotId 调用此工具。幂等：重复调用不会创建重复节点，已有节点更新语义内容并保留用户手动调整的位置，新节点自动布局。返回 createdNodeIds、updatedNodeIds、existingNodeIds、duplicateShotMappings。不触发生图、生视频或计费。用户明确要求仅分镜时不要调用任何生成工具。",
    project_get_context: "读取当前短剧项目的章节、画布、资产、镜头、候选和工作流事实。必须传入 projectId 参数。projectId 从 canvas_get_context 返回结果的 canvas.projectId 字段获取。调用顺序：先 canvas_get_context 获取 projectId，再用该 projectId 调用此工具。",
    ...
    project_create_or_update_shots: "创建或更新项目镜头业务数据，返回包含稳定 shotId 的镜头列表。这是分镜工作流的第一步：必须先调用此工具持久化镜头，再用返回的 shotId 调用 canvas_create_storyboard_shots 投影到画布。不把镜头状态写进画布 metadata。",

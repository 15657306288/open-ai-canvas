...
//   CANVAS_BROKER_GATEWAY_TOKEN  远程 Agent 侧（request/bridges/result）鉴权 token；
//                                网关经 Bearer 调用，公网暴露必须设置（默认空=不启用）
...
const GATEWAY_TOKEN = process.env.CANVAS_BROKER_GATEWAY_TOKEN ?? "";

const broker = createCanvasBridgeBroker({ gatewayToken: GATEWAY_TOKEN });
...
const server = http.createServer((req, res) => {
    ...
    // 健康/管理信息：GET / → 在线 bridge 列表（配置了网关 token 时需鉴权）
    if (url.pathname === "/" && req.method === "GET") {
        if (GATEWAY_TOKEN && !bearerOk(req.headers.authorization ?? "", GATEWAY_TOKEN)) {
            res.statusCode = 401;
        ...
        res.end(JSON.stringify({
            ...
            auth: GATEWAY_TOKEN ? "enabled" : "disabled",

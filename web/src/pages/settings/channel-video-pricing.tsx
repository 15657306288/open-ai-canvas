        ...
        <div className="mt-3 border-t border-border/70 pt-3">
            ...
            <div className="divide-y divide-border/60 rounded-md border border-border/70">
                {channel.models.map((rawModel) => {
                    ...
                    return (
                        <div key={model} className="grid gap-2 px-2.5 py-2.5 lg:grid-cols-[minmax(140px,1fr)_minmax(280px,1.6fr)_176px_160px] lg:items-center">
                            <div className="min-w-0"><div className="truncate text-xs font-medium" title={model}>{model}</div><div className="mt-0.5 truncate font-mono text-[10px] text-foreground/40" title={modelProtocolDefinition(protocol)?.create}>{modelProtocolDefinition(protocol)?.create}</div></div>
